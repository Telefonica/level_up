...
@DAPP.route("/guess", methods=['GET'])
def guess():
    return render_template("guess.html", base_address=base_address, abi_base=abi_base, network=network)

@DAPP.route("/payme", methods=['GET'])
def payme():
    return render_template("payme.html", base_address=base_address, abi_base=abi_base, network=network)

@DAPP.route("/dexknow", methods=['GET'])
def dexknow():
    return render_template("dexknow.html", base_address=base_address, abi_base=abi_base, network=network)

@DAPP.route("/gasknow", methods=['GET'])
def gasknow():
    return render_template("gasknow.html", base_address=base_address, abi_base=abi_base, network=network)

@DAPP.route("/sendtome", methods=['GET'])
def sendtome():
    return render_template("sendtome.html", base_address=base_address, abi_base=abi_base, network=network)
...
@DAPP.route("/deploy_guess", methods=['POST'])
def deploy_guess():
    if request.method == 'POST':
        if request.form.get('user_address') and request.form.get('level'):
            user_address = request.form.get('user_address')
            ...
            if exist_player:
                # Creamos contrato
                bytecode = Functions.get_instance().get_bytecode_level(level)
                bytecode_numbers = Functions.get_instance().get_bytecode_level("1005")
                abi_numbers = Functions.get_instance().get_abi_level("1005")
                abi = Functions.get_instance().get_abi_level(level)

                if (bytecode and abi) and (bytecode_numbers and abi_numbers):
                    if not Functions.get_instance().user_hash_level(network_name, level, user_address):
                        contract_numbers = w3.eth.contract(
                            abi=abi_numbers, bytecode=bytecode_numbers)
                        
                        number = Functions.get_instance().give_me_params(level)
                        print("number: {}".format(number))

                        nonce = w3.eth.getTransactionCount(address_owner)
                        transaction = contract_numbers.constructor(number).buildTransaction(
                            {"chainId": chainid, "gasPrice": w3.eth.gas_price, "from": address_owner, "nonce": nonce})
                        ...
                        # Mostrar el contract address del contrato
                        contract_address_numbers = receipt.contractAddress
                        print("Contract address numbers: {}".format(
                            contract_address_numbers))

                        # Ahora desplegamos el contrato guess
                        contract = w3.eth.contract(abi=abi, bytecode=bytecode)
                        nonce = w3.eth.getTransactionCount(address_owner)
                        param = Functions.get_instance().give_me_value()
                        print("flag: {}".format(param))

                        transaction = contract.constructor(param, contract_address_numbers, number).buildTransaction(
                            {"chainId": chainid, "gasPrice": w3.eth.gas_price, "from": address_owner, "nonce": nonce})
                        ...
                        return render_template("guess.html", base_address=base_address, abi_base=abi_base, network=network, contract_address=contract_address, abi_address=abi, contract_address_numbers=contract_address_numbers, abi_numbers=abi_numbers)
                        ...
                        contract = w3.eth.contract(
                            abi=abi, address=contract_address)
                        contract_address_numbers= contract.functions.n().call()

                        return render_template("guess.html", base_address=base_address, abi_base=abi_base, network=network, contract_address=contract_address, abi_address=abi, contract_address_numbers=contract_address_numbers, abi_numbers=abi_numbers, error=True, msg="Ya tienes un contrato de este level desplegado")
...
@DAPP.route("/deploy_payme", methods=['POST'])
def deploy_payme():
    user_address = None
    level = None
            ...
            if exist_player:
                ...
                if bytecode and abi:
                    if not Functions.get_instance().user_hash_level(network_name, level, user_address):
                        ...
                        return render_template("payme.html", base_address=base_address, abi_base=abi_base, network=network, contract_address=contract_address, abi_address=abi)
                        ...
                        return render_template("payme.html", base_address=base_address, abi_base=abi_base, network=network, contract_address=contract_address, abi_address=abi, error=True, msg="Ya tienes un contrato de este level desplegado")
...
@DAPP.route("/deploy_dexknow", methods=['POST'])
def deploy_dexknow():
    if request.method == 'POST':
        if request.form.get('user_address') and request.form.get('level'):
            user_address = request.form.get('user_address')
            ...
            if exist_player:
                # Creamos contrato (2 contratos token serán desplegados)
                print("level: {}".format(level))
                bytecode = Functions.get_instance().get_bytecode_level(level)
                bytecode_token = Functions.get_instance().get_bytecode_level("1007")
                abi_token = Functions.get_instance().get_abi_level("1007")
                abi = Functions.get_instance().get_abi_level(level)

                if (bytecode and abi) and (bytecode_token and abi_token):
                    if not Functions.get_instance().user_hash_level(network_name, level, user_address):
                        contract_token_A = w3.eth.contract(
                            abi=abi_token, bytecode=bytecode_token)
                        nonce = w3.eth.getTransactionCount(address_owner)
                        transaction = contract_token_A.constructor().buildTransaction(
                            {"chainId": chainid, "gasPrice": w3.eth.gas_price, "from": address_owner, "nonce": nonce})
                        ...
                        # Mostrar el contract address del contrato
                        contract_address_token_A = receipt.contractAddress
                        print("Contract address Token deployed: {}".format(
                            contract_address_token_A))
                        

                        # Ahora desplegamos el contrato Token B
                        contract_token_B = w3.eth.contract(
                            abi=abi_token, bytecode=bytecode_token)
                        nonce = w3.eth.getTransactionCount(address_owner)
                        transaction = contract_token_B.constructor().buildTransaction(
                            {"chainId": chainid, "gasPrice": w3.eth.gas_price, "from": address_owner, "nonce": nonce})
                        ...
                        # Mostrar el contract address del contrato
                        contract_address_token_B = receipt.contractAddress
                        print("Contract address Token deployed: {}".format(
                            contract_address_token_B))



                        # Ahora desplegamos el contrato DEX_Knowledge
                        contract = w3.eth.contract(abi=abi, bytecode=bytecode)
                        nonce = w3.eth.getTransactionCount(address_owner)
                        param = Functions.get_instance().give_me_value()
                        # amount_A, amount_B = Functions.get_instance().give_me_params(level)
                        print("flag: {}".format(param))

                        transaction = contract.constructor(contract_address_token_A, contract_address_token_B, param, 100, 200).buildTransaction(
                            {"chainId": chainid, "gasPrice": w3.eth.gas_price, "from": address_owner, "nonce": nonce})
                        ...
                        return render_template("dexknow.html", base_address=base_address, abi_base=abi_base, network=network, contract_address=contract_address, abi_address=abi, contract_address_token_A=contract_address_token_A, abi_token=abi_token, contract_address_token_B=contract_address_token_B)
                        ...
                        contract = w3.eth.contract(
                            abi=abi, address=contract_address)
                        contract_address_token_A = contract.functions.tokenA().call()
                        contract_address_token_B = contract.functions.tokenB().call()

                        return render_template("dexknow.html", base_address=base_address, abi_base=abi_base, network=network, contract_address=contract_address, abi_address=abi, contract_address_token_B=contract_address_token_B, contract_address_token_A=contract_address_token_A, abi_token=abi_token, error=True, msg="Ya tienes un contrato de este level desplegado")
...
@DAPP.route("/deploy_gasknow", methods=['POST'])
def deploy_gasknow():
    user_address = None
    level = None
            ...
            if exist_player:
                ...
                if bytecode and abi:
                    if not Functions.get_instance().user_hash_level(network_name, level, user_address):
                        ...
                        return render_template("gasknow.html", base_address=base_address, abi_base=abi_base, network=network, contract_address=contract_address, abi_address=abi)
                        ...
                        return render_template("gasknow.html", base_address=base_address, abi_base=abi_base, network=network, contract_address=contract_address, abi_address=abi, error=True, msg="Ya tienes un contrato de este level desplegado")
...
@DAPP.route("/deploy_sendtome", methods=['POST'])
def deploy_sendtome():
    user_address = None
    level = None
            ...
            if exist_player:
                ...
                if bytecode and abi:
                    if not Functions.get_instance().user_hash_level(network_name, level, user_address):
                        ...
                        return render_template("sendtome.html", base_address=base_address, abi_base=abi_base, network=network, contract_address=contract_address, abi_address=abi)
                        ...
                        return render_template("sendtome.html", base_address=base_address, abi_base=abi_base, network=network, contract_address=contract_address, abi_address=abi, error=True, msg="Ya tienes un contrato de este level desplegado")
        ...
        if request.form.get('user_address') and request.form.get('level'):
            ...
            if exist_player:
                if int(level) == 0:
                    ...
                    return redirect(url_for('deploy_guess'), code=307)
                elif int(level) == 14:
                    return redirect(url_for('deploy_payme'), code=307)
                elif int(level) == 15:
                    return redirect(url_for('deploy_dexknow'), code=307)
                elif int(level) == 16:
                    return redirect(url_for('deploy_gasknow'), code=307)
                elif int(level) == 17:
                    return redirect(url_for('deploy_sendtome'), code=307)
            else:
